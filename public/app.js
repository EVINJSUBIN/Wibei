...
const doSearch = (query) => {
    const q = query || searchInp.value.trim();
    if(q) {
        playStream(q);
        searchInp.value = '';
...
searchInp.addEventListener('input', e => {
    ...
    suggTimeout = setTimeout(async () => {
        try {
            ...
            if (data && data.length) {
                suggBox.innerHTML = data.slice(0, 5).map(item => `<div class="suggestion-item">${item[0]}</div>`).join('');
                suggBox.style.display = 'flex';
                document.querySelectorAll('.suggestion-item').forEach(el => {
                    el.addEventListener('click', () => doSearch(el.innerText));
                });
            } else {
                suggBox.style.display = 'none';
            }
